...
export default function App() {
  ...
  const [data, setData] = useState({ patients: [], staff: [], appointments: [] }); const [patient, setPatient] = useState(emptyPatient); const [booking, setBooking] = useState(emptyBooking);
  const [reschedule, setReschedule] = useState(null);
  const [message, setMessage] = useState(''); 
  const [refreshKey, setRefreshKey] = useState(0);
  

  // Simulate the data operations that the API will take over later
    useEffect(() => {
    let active = true;
  ...
  function refresh(successMessage) { setMessage(successMessage); setRefreshKey((value) => value + 1); }
  ...
  const patientMatches = data.patients.filter((item) => { const term = search.toLowerCase(); return item.fullName.toLowerCase().includes(term) || item.phone.includes(term); });
  ...
  return (<main className="shell">
    <header className="hero">
      <div><p className="eyebrow">Fictional data only</p>
      <h1>ClinicFlow</h1>
      <p>Daily appointments, patient lookup, and a safer booking workflow.</p>
      </div><label className="role-picker">Demo role<select value={role} onChange={(event) => setRole(event.target.value)}>
        <option>Receptionist</option>
        <option>Clinician</option></select>
        </label>
        </header><p className="notice">This learning prototype is not production authentication and must not contain real patient data.</p>
        {message && <p className="message" aria-live="polite">{message}</p>}
        <section className="toolbar card">
          <label>Schedule date<input type="date" value={date} onChange={(event) => { const nextDate = event.target.value; setDate(nextDate); setBooking((current) => ({ ...current, startsAt: `${nextDate}T09:00`, endsAt: `${nextDate}T09:30` })); }} />
          </label>
          <label>Search patients or appointments<input type="search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Name, phone, clinician, or reason" />
          </label></section><section className="metrics">
...
{role === 'Receptionist' ? (
  ...
  <section className="card">
    <p>Clinicians can update clinical statuses but cannot create patients, book, reschedule, or cancel appointments.</p>
  </section>
)}
...
<section className="card">
  ...
  <div className="patient-list">
    ...
    {!patientMatches.length && (
      <p>No matching fictional patients.</p>
    )}
  </div>
</section>
